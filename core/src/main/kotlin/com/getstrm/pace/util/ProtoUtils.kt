...
val DEFAULT_PAGE_PARAMETERS = PageParameters.newBuilder().setPageSize(10).setSkip(0).build()
...
/**
 * Convert a Proto Message to a JSON string, and exclude default values (Proto default values are
 * not included by default)
 */
fun GeneratedMessageV3.toJson(): String =
    JsonFormat.printer().omittingInsignificantWhitespace().preservingProtoFieldNames().print(this)

/**
 * Convert a Proto Message to a JSON string, and include default values (Proto default values are
 * not included by default)
 */
fun GeneratedMessageV3.toJsonWithDefaults(): String =
    JsonFormat.printer()
...
/**
 * Accepts JSON, YAML, or base64 encoded JSON or YAML, and converts it into a Proto Message of type
 * [T]
 */
inline fun <reified T : GeneratedMessageV3> String.toProto(validate: Boolean = true): T {
    ...
    // As the constructors of generated proto messages are private, we need to set it accessible
    // first
...
/** Accepts JSON, YAML, or base64 encoded JSON or YAML and converts it to a JSON string */
            ...
            BadRequest.newBuilder()
                .addFieldViolations(
                    BadRequest.FieldViolation.newBuilder()
                        .setDescription(
                            "Could not parse payload as JSON, YAML or base64 encoded JSON or YAML: ${e.message}"
                        )
                        .build()
                )
                .build()
...
/** force case type string to TransformCase don't allow TRANSFORM_NOT_SET */
...
fun GlobalTransform.refAndType(): Pair<String, GlobalTransform.TransformCase> =
    Pair(
        when (transformCase) {
            GlobalTransform.TransformCase.TAG_TRANSFORM -> tagTransform.tagContent
            null,
            GlobalTransform.TransformCase.TRANSFORM_NOT_SET ->
                throw BadRequestException(
                    code = BadRequestException.Code.INVALID_ARGUMENT,
                    badRequest =
                        BadRequest.newBuilder()
                            .addFieldViolations(
                                BadRequest.FieldViolation.newBuilder()
                                    .setDescription(
                                        "A GlobalTransform was seen with no `transform`. The GlobalTransform was: " +
                                            this.toJson()
                                    )
                                    .build()
                            )
...
fun Descriptors.Descriptor.getJSONSchema(): String {
    ...
    return object {}.javaClass.getResource("/jsonschema/$directory/$file")?.readText()
...
fun DataPolicy.Source.toDDL(): String {
    val fields =
        this.fieldsList.joinToString(separator = ",\n") { field ->
