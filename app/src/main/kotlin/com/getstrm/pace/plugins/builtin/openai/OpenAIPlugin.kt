...
class OpenAIPlugin(
    ...
    inner class GenerateDataPolicy : GenerateDataPolicyAction {
        override val payloadJsonSchema: String =
            OpenAIDataPolicyGeneratorPayload.getDescriptor().getJSONSchema()
        ...
        override suspend fun invoke(payload: String): DataPolicy {
            ...
            return try {
                when (generatorPayload.dataPolicyCase) {
                    ...
                    OpenAIDataPolicyGeneratorPayload.DataPolicyCase.DATAPOLICY_NOT_SET,
                    null -> throw invalidPayloadTypeNotSet()
                }
        ...
        private suspend fun generate(instructions: String, dataPolicy: DataPolicy): DataPolicy {
            val systemMessage =
                ChatMessage(
                    ...
                    content =
                        "Your task is to create a YAML file based on a JSON Schema and instructions by the end user. Ensure that you only respond with the YAML as plain text, do not provide any other details.",
                )

            val jsonSchemaMessage =
                ChatMessage(
                    role = ChatRole.User,
                    content = dataPolicyJsonSchema,
                )

            val blueprintDataPolicyMessage =
                ChatMessage(
                    role = ChatRole.User,
                    content =
                        """
            ...
            val request =
                ChatCompletionRequest(
                    ...
                    seed =
                        42, // https://platform.openai.com/docs/guides/text-generation/reproducible-outputs
                    maxTokens = 4095,
                    temperature = 0.0,
                    topP = 1.0,
    ...
    inner class GenerateSampleData : GenerateSampleDataAction {
        override val payloadJsonSchema: String =
            OpenAISampleDataGeneratorPayload.getDescriptor().getJSONSchema()
            ...
            return try {
                when (generatorPayload.sourceDetailsCase) {
                    OpenAISampleDataGeneratorPayload.SourceDetailsCase.SOURCE ->
                        generateData(
                            generatorPayload.source,
                            generatorPayload.additionalSystemInstructionsList
                        )
                    OpenAISampleDataGeneratorPayload.SourceDetailsCase.SOURCEDETAILS_NOT_SET,
                    null -> throw invalidPayloadTypeNotSet()
                }
            } catch (e: InvalidProtocolBufferException) {
                throw invalidPayload(e)
        ...
        private suspend fun generateData(
            ...
            val systemMessage =
                ...
                """
                            .trimIndent() +
                            additionalSystemInstructions.joinToString(
                                prefix = "-",
                                separator = "\n"
                            ),
                )

            val ddlMessage = ChatMessage(role = ChatRole.User, content = source.toDDL())
    ...
    private fun invalidPayload(e: InvalidProtocolBufferException) =
        BadRequestException(
    ...
    private fun invalidPayloadTypeNotSet() =
        BadRequestException(
            BadRequestException.Code.INVALID_ARGUMENT,
            BadRequest.newBuilder()
                .addFieldViolations(
        ...
        return when (val messageContent = response.choices.first().message.messageContent) {
            is TextContent -> {
                try {
                    ...
                    throw InternalException(
                        ...
                        DebugInfo.newBuilder()
                            .setDetail(
                                "Could not parse DataPolicy YAML generated by OpenAI: ${e.message}"
                            )
                            .build(),
                        e,
                    )
                ...
                throw InternalException(
                    InternalException.Code.INTERNAL,
                    DebugInfo.newBuilder()
                        .setDetail("Unexpected OpenAI response: $messageContent")
                        .build(),
                )
            }
        }
