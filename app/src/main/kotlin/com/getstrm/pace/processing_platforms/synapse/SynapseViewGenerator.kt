...
class SynapseViewGenerator(dataPolicy: DataPolicy, customJooqSettings: Settings.() -> Unit = {}) :
                ...
                ruleSet.uniquePrincipals().map {
                    DSL.query(
                        jooq
                            ...
                            .on(DSL.table(DSL.unquotedName(renderName(viewName))))
    ...
    fun dropViewsSQL() =
        jooq.queries(
            dataPolicy.ruleSetsList.map { jooq.dropViewIfExists(renderName(it.target.fullname)) }
        )

    /**
     * If the database name is included in the ref, we need to drop it. The sql query for Synapse
     * only supports <schema.table>.
     */
        ...
        return if (principals.isEmpty()) {
            ...
            DSL.or(
                principals.map { principal ->
                    when {
                        principal.hasGroup() ->
                            DSL.condition("IS_ROLEMEMBER({0})=1", principal.group)
                        else ->
                            throw InternalException(
                                InternalException.Code.INTERNAL,
                                DebugInfo.newBuilder()
                                    .setDetail(
                                        "Principal of type ${principal.principalCase} is not supported for platform Synapse. $UNIMPLEMENTED"
                                    )
                                    .build()
                            )
                    }
    ...
    /**
     * Synapse does not support the direct use of booleans in the where clause. The conditions need
    ...
    override fun toCondition(filter: GenericFilter): Condition {
        ...
        builder.conditionsBuilderList.map {
            ...
            it.condition =
                DSL.condition(
                        DSL.`when`(DSL.condition(synapseCondition), 1).otherwise(0).toString()
                    )
                    .toString()
        }
        with(builder.build()) {
    ...
    private fun DataPolicy.RuleSet.Filter.RetentionFilter.Condition.toSynapseRetentionCondition():
        JooqField<Int> {
