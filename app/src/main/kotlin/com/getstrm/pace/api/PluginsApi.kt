...
@GrpcService
class PluginsApi(private val pluginsService: PluginsService) :
    PluginsServiceGrpcKt.PluginsServiceCoroutineImplBase() {
    override suspend fun listPlugins(request: ListPluginsRequest): ListPluginsResponse =
        ListPluginsResponse.newBuilder().addAllPlugins(pluginsService.listPlugins()).build()

    override suspend fun getPayloadJSONSchema(
        request: GetPayloadJSONSchemaRequest
